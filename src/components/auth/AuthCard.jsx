...
export const AuthCard = ({ onNavigateToExplore, onForgotPassword }) => {
  ...
  //khi đổi tab (URL đổi) -> xoá alert cũ
  useEffect(() => {
    setAlertInfo(null);
  }, [authMode])
  ...
  //chuyen tab bang navigate - doi url
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        setAlertInfo({
          type: 'success',
          message: `Đăng nhập thành công với ${formData.email}!`,
        });
        //chuyển trang sau khi mà xác nhận password và email đúng
        const redirectTo = location.state?.from?.pathname || '/otp-vertify';
        setTimeout(() => navigate(redirectTo, { replace: true }), 800);
      } else {
        //register từ authcontext
        await register(formData.firstName, formData.lastName, formData.phone, formData.address, formData.email, formData.password);
        setAlertInfo({
          type: 'success',
          message: `Đăng ký thành công cho ${formData.firstName} ${formData.lastName} !`,
        });
        setTimeout(() => {
          navigate('/auth/login')
        })
      ...
      setAlertInfo({ type: 'danger', message });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSocialLogin = (provider) => {
    setAlertInfo({
      type: 'info',
      message: `Chức năng đăng nhập ${provider} đang được phát triển.`,
    });
  };

  return (
    <div className="container-flui d-flex justify-content-center align-items-center w-100">
      <div className="bg-white rounded-4 shadow-sm border overflow-hidden position-relative w-100" style={{ maxWidth: '800px' }}>
        <div className="p-4 p-sm-5">
          ...
          <div className="d-flex justify-content-center border-bottom mb-4">
            <div className="d-flex gap-4">
              <button
                type="button"
                id="tab-auth-login"
                className={`btn btn-link text-decoration-none px-2 pb-2 fw-semibold position-relative ${authMode === 'login' ? 'text-primary' : 'text-secondary'
                  }`}
                style={{
                  fontSize: '15px',
              ...
              <button
                type="button"
                id="tab-auth-signup"
                className={`btn btn-link text-decoration-none px-2 pb-2 fw-semibold position-relative ${authMode === 'signup' ? 'text-primary' : 'text-secondary'
                  }`}
                style={{
                  fontSize: '15px',
          ...
          <form onSubmit={handleSubmit}>
            ...
            {authMode === 'signup' && (
              <>
                <div className="mb-3">
                  <label htmlFor="auth-first-name" className="form-label text-dark small fw-medium mb-1">
                    First Name
                  </label>
                ...
                <div className="mb-3">
                  <label htmlFor="auth-last-name" className="form-label text-dark small fw-medium mb-1">
                    Last Name
                  </label>
                  <div className="input-group">
                    <span className="input-group-text bg-white text-secondary border-end-0">
                ...
                <div className="mb-3">
                  <label htmlFor="auth-phone" className="form-label text-dark small fw-medium mb-1">
                    Phone
                  </label>
                  <div className="input-group">
                    <span className="input-group-text bg-white text-secondary border-end-0">
                ...
                <div className="mb-3">
                  <label htmlFor="auth-address" className="form-label text-dark small fw-medium mb-1">
                    address
                  </label>
                  <div className="input-group">
                    <span className="input-group-text bg-white text-secondary border-end-0">
