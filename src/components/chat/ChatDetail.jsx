...
export const ChatDetail = ({ hotel, onBack, onOpenDetails, isEmbedded = false }) => {
  ...
  return (
    ...
    style={{ backgroundColor: '#f4f7fa' }}>
      <div className={`bg-white ${!isEmbedded ? 'rounded-4 shadow-sm border' : ''} overflow-hidden position-relative d-flex flex-column`} style={{
        height: isEmbedded ? '600px' : '650px',
        maxWidth: '650px',
        width: '100%'
      }}>
        {/* 1. Header */}
          ...
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`p-3 rounded-4 shadow-sm position-relative ${isUser
                    ? 'text-white'
